...
def is_spotify_authenticated(session_id):
    tokens = get_user_token(session_id)
    if tokens:
        print(str(tokens.access_token)[:5]+"*****"+str(tokens.access_token)[-5:],
              tokens.expires_in - timezone.now())
...
def refresh_spotify_token(session_id):
    refresh_token = get_user_token(session_id=session_id).refresh_token
    ...
    update_or_create_user_token(session_id=session_id, access_token=access_token,
...
def execute_spotify_api_request(session_id, endpoint, post_=False, put_=False):
    ...
    try:
        ...
        return {'Error': 'Sorry, Issue with request.'}
